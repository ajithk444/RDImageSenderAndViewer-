  ...
  onSubmitMultipleItems() {
    if (this.areAllItemsFieldsFilled) {
      ...
      for (let index = 0; index < this.filesAttached.length; index++) {
        ...
        console.log('file type: ', this.currentFile.name.split('.').pop());
        if (this.currentFile.name.split('.').pop() === 'dcm') {
          this.itemService.addItemFilesToStorage(this.item, this.user.uid, this.currentFile, index, id).subscribe(tagsValue => {
            if (tagsValue) {
              ...
              if (tagsValue.tagsFound && tagsValue.lossyImageCompression === '00') {
                if (tagsValue.laterality.trim() === 'L' && tagsValue.imageComments.trim() === 'Macula') {
                ...
                } else if (tagsValue.laterality.trim() === 'L' && tagsValue.imageComments.trim() === 'DiscoOptico') {
                  this.imageIndexes[1] = tagsValue.url;
                  this.checkboxOIED = true;
                } else if (tagsValue.laterality.trim() === 'R' && tagsValue.imageComments.trim() === 'Macula') {
                  this.imageIndexes[2] = tagsValue.url;
                  this.checkboxODEM = true;
                } else if (tagsValue.laterality.trim() === 'R' && tagsValue.imageComments.trim() === 'DiscoOptico') {
